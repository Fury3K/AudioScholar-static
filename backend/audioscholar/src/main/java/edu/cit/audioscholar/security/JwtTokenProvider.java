...
public class JwtTokenProvider {
	...
	public void init() {
		if (jwtSecretString == null || jwtSecretString.isBlank() || jwtSecretString.length() < 43) {
			logger.warn(
					"JWT Secret ('app.jwt.secret') is not configured, is blank, or too short (requires >= 43 Base64 chars for HS256)! Using a default temporary key. PLEASE CONFIGURE a strong secret.");
			...
			try {
				...
				logger.error(
						"Invalid Base64 encoding for JWT secret ('app.jwt.secret'). Using a default temporary key. PLEASE FIX the configuration.",
						e);
				this.jwtSecretKey = Jwts.SIG.HS256.key().build();
			}
		}
	...
	public String generateToken(Authentication authentication) {
		...
		return Jwts.builder().id(jti).subject(username).claim("roles", authorities)
				.claim("name", displayName != null ? displayName : "").issuedAt(now).expiration(expiryDate)
				.signWith(getJwtSecretKey(), Jwts.SIG.HS256).compact();
	}

	public String getUserIdFromJWT(String token) {
		Claims claims = Jwts.parser().verifyWith(getJwtSecretKey()).build().parseSignedClaims(token).getPayload();
